...
async function getBookingById(id) {
    ...
    if (mongoose.isValidObjectId(id)) {
        booking = await BookingModel.findById(id).lean();
...
async function updateBooking(id, updateData) {
    ...
    const booking = await BookingModel.findOneAndUpdate(query, { $set: updateData }, { new: true }).lean();
    return booking;
}

async function deleteBooking(id) {
    const query = mongoose.isValidObjectId(id) ? { _id: id } : { bookingId: id };
    const result = await BookingModel.findOneAndDelete(query);
    return !!result;
}
